...
| id | status | item |
|----|--------|------|
| R1 | done | **Apply the menu black-sprite fix.** Root cause confirmed in `94fce45`. Closed by adding 4-bit `colorWriteMask` to `RenderStateVector` / `Blend_Key()`, wiring `rt.RenderTargetWriteMask = rs.colorWriteMask` in `D3D11States.cpp`, exposing `Set_Color_Write_Mask` on `IRenderBackend`/`D3D11Backend`, and syncing `D3DRS_COLORWRITEENABLE` in `DX8Wrapper::Set_DX8_Render_State`. Verified via `w3d_d3d11_smoke` check U (alpha-only write mask leaves RGB clear magenta intact) and cache check O (distinct blend states). |
...
| R10 | done | **Watch for PR #2613 merging into upstream `main`.** Merged upstream on 2026-08-29 at commit `0610e331` (28 commits total). Interface target is now stable upstream. Opens path to revisit R8 lifecycle/factory alignment and closed the R14 divergence. |
...
| R14 | done | **Adopt the `End_Scene(bool flip_frame = true)` default from `0610e331`.** Closed by adopting the `= true` default in `IRenderBackend.h:167`. Fulfills public promise on PR #2613 (comment 5447357664). |
| R15 | open | **VC6 CI was red for two weeks and nobody was blocked by it — eight separate walls, not one.** `vc6`/`vc6-profile` failed on every build-triggering push since the D3D11 backend landed; the green runs in between were docs-only pushes the path filter skipped, so the red never surfaced. PR #2 turns all seven vc6 jobs green for the first time. In order: `<unordered_map>` in `D3D11States.h` · `std::`-qualified C library calls in `RenderBackend.cpp` (VC6 puts none of it in `std`, and its STLport `<cstdio>` collides with the tree's own `vsnprintf` shim) · `<cstdint>` in the parity differ · `unsigned long long` and `GetTickCount64` in the DX8 frame dump · `d3d11.lib` still on the game link line · a leaked for-scope in the Stratagem AI (`error C2374`) · an array-of-class brace initializer in a `RTS_DEBUG` self-test (`error C2440`) · and ten D3D11 mirror hooks called from `dx8wrapper.h` inlines whose bodies live in an excluded translation unit (`LNK2001`). Approach: the D3D11 backend and its two oracles are gated on `NOT IS_VS6_BUILD`, and the one backend file vc6 still compiles carries no-op hook bodies behind `W3DNEXT_HAS_D3D11`. **Row stays open for the standing question it exposed:** nothing here watches CI, so a red main was only noticed by accident, and the fork's own code is only VC6-clean as far as a build has reached — six of the eight walls were in code VC6 had never compiled. |
...
| P3 | done | **Verify the weekly automation actually fired.** Verified 2026-09-07: Issue #1 (Weekly triage digest) posted automatically to `eydotan/W3DNext` on 2026-09-07T07:18:44Z; CI green. |
...
| P6 | done | **Discord account ban appeal** filed 2026-08-14 with Trust & Safety. Appeal accepted and account unlocked on 2026-09-07. Per AGENT-CC.md doctrine, Discord access remains operator-manual only. |
