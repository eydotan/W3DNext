...
| R14 | open `promise` | **Adopt the `End_Scene(bool flip_frame = true)` default from `0610e331`.** Committed to publicly on PR #2613 (comment 5447357664): *"`End_Scene(bool flip_frame = true)` in `0610e331` is a new one I'll pick up."* Unlike the `Clear` and `Set_Gamma` restorations in that run, this one is a genuine divergence rather than a convergence — upstream's `IRenderBackend.h:58` carries the default and this tree's `:167` declares `End_Scene(bool flip_frame)` with none. Cheap edit; the obligation is what makes it a row. |
