...
| R13 | discuss | **Dropping the backend `Initialize`/`Shutdown` pair removes the extension point the interface exists for — report upstream?** `830cca12` deletes `virtual void Initialize(void * window, int width, int height)` and `virtual void Shutdown()` from `IRenderBackend` as unused, and the hunk deletes the comment that justified them: *"A backend that drives its own device creates it in Initialize and releases it in Shutdown; the DX8 reference backend leaves them as no-ops."* That describes a backend that does not exist upstream and does exist here: this tree's `D3D11Backend.h:70-71` overrides both, with live callers at `dx8wrapper.cpp:398` (`g_renderBackend->Initialize(_Hwnd, ResolutionWidth, ResolutionHeight)`) and `Backend/RenderBackend.cpp:110` (`g_renderBackend->Shutdown()`). Same shape as the `Set_Gamma` finding in R7 — no-routed-caller and no-caller-needs-it come apart — but a step stronger, since a no-op-for-DX8 hook is by construction only exercised by a second backend, so "unused" here is a measurement of there being one backend rather than of the hook being dead. Decide whether to post it (public, needs a green light) and whether to keep the pair locally regardless. **Do not take `830cca12`.** |
