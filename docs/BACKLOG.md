...
| R15 | open | **VC6 CI jobs were red for two weeks and nobody was blocked by it.** `vc6`/`vc6-profile` failed on every build-triggering push since the D3D11 backend landed (`D3D11States.h(38) : fatal error C1083: Cannot open include file: 'unordered_map'`); the green runs in between were docs-only pushes the path filter skipped, so the red never surfaced. Fixed by gating the D3D11 sources and the smoke oracle on `NOT IS_VS6_BUILD` and compiling `RenderBackend.cpp` without them behind `W3DNEXT_HAS_D3D11` (PR #2). Row stays open for the standing question it exposed: nothing here watches CI, so a red main is only noticed by accident. |
